...
<section class="container mx-auto">
	<div class="relative h-[650px] min-w-full bg-white-600 max-sm:mb-12 sm:mt-8 sm:rounded-xl">
		{#if !editMode}
			<div
				class="embla absolute z-40 h-full w-full sm:overflow-hidden"
			...
			>
				<div class="embla__container flex h-full w-full">
					{#each signatureImageCache.filter((e) => e != undefined) as slide, i}
						...
						<div
							class="embla__slide relative z-30 min-w-0 font-sans"
							style="flex: 0 0 100%"
						...
						>
							...
							<div class="sm:text-outline absolute -bottom-16 left-10 z-40 h-12 sm:bottom-8">
		...
		{/if}
		<div bind:this={box} class="h-full w-full overflow-hidden">
			<canvas bind:this={canvas} class="absolute z-40 m-0 p-0" width="1536" height="650" class:hidden={!editMode}
			></canvas>
		...
		{#if !editMode}
			<div class="absolute bottom-0 right-10 z-50 flex h-12 items-center gap-3 sm:bottom-8">
				...
				<button
					...
					class="absolute size-12 rounded-full bg-white-700 transition hover:bg-white-700 active:bg-white-600 max-sm:-bottom-16 max-sm:right-0 sm:relative sm:bg-white-500"
					onclick={enableEditMode}
				>
					<span
						class="sm:text-outline absolute right-full w-max text-center font-hand max-sm:hidden sm:-left-16 sm:-top-16 sm:rotate-6"
					>
					...
					<span
						class="sm:text-outline pointer-events-none absolute bottom-full right-10 -rotate-6 -scale-x-100 max-sm:hidden"
					>
