<script lang="ts">
	...
	let emblaPlugins = [Autoplay({ playOnInit: false, delay: 6000 })];
...
<section class="container mx-auto">
	<div class="relative h-[650px] min-w-full bg-white-600 max-sm:mb-12 sm:mt-8 sm:rounded-xl">
		<div
			...
			onemblaInit={onInit}
		>
			<div class="embla__container text-outline flex h-full w-full">
				{#each signatureImageCache.filter((e) => e != undefined) as slide, i}
					{@const date_created = new Date(slide.ts_created)}
					<div
						class="embla__slide relative min-w-0 font-sans"
						style="flex: 0 0 100%"
					...
					>
						...
						<div class="absolute -bottom-16 left-10 z-40 h-12 sm:bottom-8">
		...
		<div class="absolute bottom-0 right-10 z-50 flex h-12 items-center gap-3 sm:bottom-8">
			{#each signatures as signature, i}
				<button
					id={signature.id}
					class="size-2 rounded-full bg-white-700 transition hover:scale-125 hover:bg-grey-800 max-md:hidden"
			...
			>
				...
				<span
					class="text-outline pointer-events-none absolute bottom-full right-10 -rotate-6 -scale-x-100 max-sm:hidden"
				>
