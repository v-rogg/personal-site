...
import { registerScrollHandler, unregisterScrollHandler } from "$lib/carousel-state";
...
function renderSignatureToImage(sig: Signature): string {
	const sigData = JSON.parse(decompress(sig.signature));
	signatureBuildPad.fromData(uncenterSignature(sigData));
	return signatureBuildPad.toDataURL();
}

function loadPrev() {
	emblaApi.scrollPrev();
	emblaApi.plugins().autoplay.stop();
	trackEvent("click.signatures.previous");
}

function loadNext() {
	emblaApi.scrollNext();
	emblaApi.plugins().autoplay.stop();
	trackEvent("click.signatures.next");
}

function scrollToSignatureById(id: string) {
...
onMount(async () => {
	// Register handler so Analytics can scroll to a specific signature
	registerScrollHandler(scrollToSignatureById);
	...
	// First, render preloaded signatures (SSR data) immediately
	for (let i = 0; i < preloadedSignatures.length && i < signatures.length; i++) {
		const sig = preloadedSignatures[i];
		if (sig && sig.id === signatures[i].id) {
			signatureImageCache[i] = { ...sig, image: renderSignatureToImage(sig) };
			// Also cache in localStorage for future visits
			localStorage.setItem(`vr-www.signature.${sig.id}`, JSON.stringify(sig));
		}
	}

	// Then load remaining signatures in background
	const startIndex = preloadedSignatures.length;
	for (let i = startIndex; i < signatures.length; i++) {
		...
		if (cache) {
			sig = JSON.parse(cache);
		} else {
			sig = await getSignature(signature.id);
			if (sig) {
				localStorage.setItem(`vr-www.signature.${signature.id}`, JSON.stringify(sig));
			}
		}
...
<div
	...
	{#if autoplayState}
		<button
			...
			onclick={() => {
				emblaApi.plugins().autoplay.stop();
				trackEvent("click.signatures.autoplay.stop");
			}}
			in:blur={{ amount: 1 }}
	...
	{:else}
		<button
			...
			onclick={() => {
				...
				trackEvent("click.signatures.autoplay.start");
			}}
			in:blur={{ amount: 1 }}
			class="relative mr-2 size-2 max-sm:hidden">
	...
	{#each signatures as signature, i}
		<button
			...
			onclick={() => {
				emblaApi.scrollTo(i);
				emblaApi.plugins().autoplay.stop();
				trackEvent("click.signatures.carousel.skip");
			}}
