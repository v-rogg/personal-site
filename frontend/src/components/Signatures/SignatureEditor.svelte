...
let { signatures = $bindable(), closeEditMode } = $props();
...
penWidthMultiplicator.subscribe((v) => {
	if (tool === Tools.pen && signaturePad) {
...
function undo() {
	history.pop();
	signaturePad.fromData(uncenterSignature(history));
	if (colorPicker) trackEvent("click.signatures.colorPicker.close", { color: penColor });
	colorPicker = false;
	trackEvent("click.signatures.editor.undo");
}
...
function clear() {
	...
	if (colorPicker) trackEvent("click.signatures.colorPicker.close", { color: penColor });
	colorPicker = false;
	trackEvent("click.signatures.editor.clear");
}
...
function enablePen() {
	if (tool !== Tools.pen) {
		...
		trackEvent("click.signatures.editor.pen");
	} else {
		colorPicker = !colorPicker;
		if (colorPicker) {
			trackEvent("click.signatures.colorPicker.open");
		} else {
			trackEvent("click.signatures.colorPicker.close", { color: penColor });
		}
	}
}
...
function enabledErase() {
	...
	if (colorPicker) trackEvent("click.signatures.colorPicker.close", { color: penColor });
	colorPicker = false;
	trackEvent("click.signatures.editor.eraser");
}
...
function centerSignature(data: PointGroup[]): PointGroup[] {
	if (box) {
		...
		for (let signature of data) {
			...
			for (let point of centeredSignature.points) {
...
async function handleSubmit(e: Event) {
	e.preventDefault();
	if (!nameInput || history.length === 0) return;
	...
	try {
		const result = await createSignature({
			...
			signature: JSON.stringify(history)
		});

		signatureSentId = result.id;
		signatures.unshift({
			id: signatureSentId,
			name: nameInput,
			ts_created: Date.now()
		...
		trackEvent("click.signatures.saveDialog.save");
	} catch (error) {
		console.error("Failed to save signature:", error);
		addToast({
			data: {
				title: "Fehler beim Speichern",
				description: "fa-solid fa-exclamation-triangle"
			}
		});
	} finally {
...
function handleClose() {
	if (signatureSentId) {
		const signatureUrl = `${window.location.origin}/?s=${signatureSentId}`;
		window.history.replaceState({}, "", signatureUrl);
	}
	$open = false;
	closeEditMode();
}
...
<canvas
	...
	width="1536"
	height="650"
	transition:blur={{ amount: 10, duration: 600, easing: expoInOut }}></canvas>
<div
	...
	<button
		...
		onclick={() => {
			$open = true;
			trackEvent("click.signatures.editor.openSaveDialog");
		}}
		disabled={history.length === 0}>
			...
			{:else}
				...
				<fieldset>
					...
					<button
						...
						onclick={() => {
							...
							trackEvent("click.signatures.editor.clipboard.text");
						}}>
						{signatureUrl}
				...
				<div class="mt-6 flex justify-start gap-4 min-[450px]:flex-row-reverse">
					<button
						...
						onclick={() => {
							toClipboard(signatureUrl);
							trackEvent("click.signatures.editor.clipboard.button");
						}}
						type="submit">
						Link kopieren
					</button>
