...
import { scrollToSignature as scrollCarouselToSignature } from "$lib/carousel-state";
...
function scrollToSignature(id: string) {
	// Scroll to top of page (signatures section)
	window.scrollTo({ top: 0, behavior: "smooth" });

	// Navigate carousel to specific signature
	setTimeout(() => {
		scrollCarouselToSignature(id);
	}, 300); // Small delay to let scroll animation start
}
</script>
	...
	<div class="flex grid-cols-1 flex-col gap-4 xl:grid xl:grid-cols-8">
		...
		<div class="group col-span-2 relative flex flex-col bg-white-600 pl-10 pr-10 pt-9 pb-10 backdrop-blur transition duration-500 hover:shadow-xl sm:rounded-xl">
			...
			<span
				...
				<svg width="28" viewBox="0 0 67 32" xmlns="http://www.w3.org/2000/svg" class="fill-gray-600">
					<path d="M47.077,26.077C13.31,30.875 0.2,2.975 0.2,2.975C-0.291,1.935 0.153,0.692 1.193,0.2C2.232,-0.291 3.475,0.153 3.967,1.193C3.967,1.193 15.872,26.16 46.303,21.976L45.626,18.386C45.47,17.559 45.73,16.709 46.321,16.11C46.913,15.512 47.761,15.242 48.589,15.389L64.622,18.228C65.671,18.413 66.495,19.229 66.693,20.275C66.89,21.321 66.42,22.381 65.511,22.936L51.615,31.423C50.897,31.862 50.009,31.92 49.24,31.578C48.471,31.237 47.919,30.539 47.763,29.712L47.077,26.077Z"></path>
				</svg>
			</span>
		</div>

		<!-- Longest Painting Duration -->
		{#if stats.drawing_durations.length > 0}
			<button
				onclick={() => scrollToSignature(stats.drawing_durations[0].id)}
				class="group col-span-2 relative flex flex-col bg-white-600 pl-10 pr-10 pt-9 pb-10 backdrop-blur transition duration-500 hover:shadow-xl sm:rounded-xl cursor-pointer text-left"
			>
				<div class="text-center">
					<div class="text-xl font-semibold text-black mb-2">
						Längste Mal-Session
