...
let { blog, tags }: { blog: BlogEntry[]; tags: string[] } = $props();
...
const gridColSeries = [3, 6, 4, 5, 6, 3, 4, 5];

function checkTags(data: BlogEntry["data"]) {
	if (data?.tags) {
...
<section id="blog" class="mt-20">
				...
				{#if filterOpen}
					<button
						...
						onclick={() => {
							...
							trackEvent("click.blog.tags.tag", { tag: tag });
						}}
					>
						{tag}
					</button>
	...
	<div class="flex grid-cols-1 flex-col gap-4 xl:grid xl:grid-cols-9">
		{#each blog as entry, i}
			<Post
				url={`/blog/${entry.id}`}
				cls="col-span-{gridColSeries[i % 8]} max-lg:col-span-1 {checkTags(entry.data)
					? 'opacity-100'
